...
if (!empty($_POST['course'])) {
    $newCourses = implode(',', $_POST['course']);

    // Retrieve existing courses from the 'user' table
    $result = mysqli_query($conn, "SELECT id FROM user WHERE user_id='$user_id'");
    $row = mysqli_fetch_assoc($result);
    $existingCourses = $row['id'];

    // Combine the existing and new courses, and remove any duplicate values
    $combinedCourses = array_unique(array_merge(explode(',', $existingCourses), explode(',', $newCourses)));

    // Convert the combined array of IDs into a comma-separated string
    $courseString = implode(',', $combinedCourses);

    // Update the 'user' table with the new data
    $sql = mysqli_query($conn, "UPDATE user SET id='$courseString' WHERE user_id='$user_id'");
    // if ($sql) {
    ...
    // echo "No input received from the form.";
}
?>
...
<form method="post" action="matching2.php" style="padding: 20px;">
<table style="width: 100%;">
<tr><th>Registered Course</th>
...
<center>
<button type="submit" style="margin: 20px; width: 200px; height: 50px;">Start Matching</button>
<a href="Syllabus.php"><button type="button" style="color: black; margin: 20px; width: 200px; height: 50px;" >Edit</button></a>
<!-- <a href="Account.php"><button type="button" style="color: black; margin: 20px; width: 200px; height: 50px;" >Account</button></a> -->
</center>
</form>
</div></div></section>
<br>
<footer class='u-align-center u-clearfix u-footer u-grey-80 u-footer' id='sec-a21e'><div class='u-clearfix u-sheet u-valign-middle u-sheet-1'><p class='u-small-text u-text u-text-variant u-text-1'>2023 Job Recommender</p></div></footer></body></html>
